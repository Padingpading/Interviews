...
// *
// */
//package com.padingpading.interview.jvm.classloader.source;
//
...
// *   "java.security.KeyStore$Builder$FileBuilder$1" 内部类.内部类
// *   "java.net.URLClassLoader$3$1" URLClassLoader第三个内部类中的第一个内部类。
// * </pre></blockquote>
// *
// * @see      #resolveClass(Class)
// * @since 1.0
// */
////1、从给定的全限定类名,转换为文件名字,读取全限定类名指向的class文件。
////2、class包含对classloader的引用。
////3、数组类的class对象不是由classloader生成,由jvm运行时动态生成,数组类型的classloader和元素的classloader是一样的。
////4、ClassLoader在尝试寻找类加载器之前,会委托给父类加载器先进行加载。
////5、ClassLoader创建的对象,对象方法中引用的其他类,也是由当前的ClassLoader进行加载。
////definClass 会通过二进制流的方式加载类,

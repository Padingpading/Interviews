...
//
///**
// * A {@link java.util.concurrent.Future} that may be explicitly completed (setting its
...
// * </ul>
// *
// * <p>CompletableFuture also implements {@link java.util.concurrent.Future} with the following
// * policies: <ul>
// *
...
// * 实现接口:
// *  Future:实现异步获取结果值。
// *  CompletionStage: 多任务的编排方法,主要参数为异步回调的函数。Function、Consumer、
// *
// * @author Doug Lea
...
// * @since 1.8
// */
//public class CompletableFuture11<T> implements java.util.concurrent.Future<T>, CompletionStage<T> {
//
//    /*
//     * Overview:
//     *
...
//    // 依赖操作的堆栈顶部
//    //stack 是一个链栈，存放与this对象直接关联的Completion对象（也就是上文说到的completion对象）。
//    // Completion对象是用来驱动某一个CompletableFuture对象（比如上文 task2CompletableFuture），
//    // 所谓的驱动，就是使得这个CompletableFuture对象的result成员变为非null。
...
//            if (STACK.compareAndSet(f, h, t = h.neaxt)) {
//                if (t != null) {
//                    if (f != this) {
//                        pushStack(h);
//                        continue;
...
//        //异步任务执行器，如果为空则有主线程执行任务不进行异步执行。
//        Executor executor;                 // executor to use (null if none)
//        //指向当前任务构建的CompletabueFuture
//        CompletableFuture<V> dep;          // the dependent to complete
//        //指向源CompletableFuture任务
...
//                // 帮助gc
//                dep = null; fn = null;
//                // 当d还没有完成的时候才开始执行，避免重复执行 result被volatile修饰
//                if (d.result == null) {
//                    try {
//                        f.run();
//                        /**
...
//     * 返回一个新的CompletableFuture,该future由运行在
//     * {@link ForkJoinPool#commonPool()}中的任务异步完成
//     * 值通过调用给定的Supplier获得。
...
//     * 返回一个新的CompletableFuture,该futrue由给定的执行器中执行的任务异步完成,
//     * 值通过调用给定的Supplier获得
//     * @param supplier a function returning the value to be used
//     * to complete the returned CompletableFuture
...
//    }
//
//    /* 异步执行supply,返回结果值为空的CompletableFuture
//     */
...
//     * 返回一个新的CompletableFuture,该futrue由运行给定操作后在{@link ForkJoinPool#commonPool()}
//     * 中运行任务异步执行完成。
...
//     * 该future由在给定执行器中运行的任务在运行给定操作后异步完成。
//     * @param runnable the action to run before completing the
//     * returned CompletableFuture
//     * @param executor the executor to use for asynchronous execution
//     * @return the new CompletableFuture
...
//    }
//
//    /**不会传入 Executor，因为它优先让当前线程（例子中是 main 线程）来执行后续 stage 的 task。
//     * @param fn
//     * @param <U>
//     * @return
//     */
...
//            return new CompletableFuture.MinimalStage<U>(); }
...
//                unipush(new CompletableFuture.UniRelay<T,T>(d, this));
//                return d;
//            }
//        }
//    }
